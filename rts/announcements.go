...
func AmpAnnouncementsHandler(w http.ResponseWriter, r *http.Request) {
	// vars := mux.Vars(r){{end}}
	// lang := vars["lang"]
	// site := mod.Site{}
...
func AdminAnnouncementsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !hlp.IsEmpty(userName) {
		records, err := JDB.ReadAll("announcements")
		if err != nil {
			fmt.Println("Error", err)
		}
		posts := []mod.Announcement{}
		for _, f := range records {
			postFound := mod.Announcement{}
			if err := json.Unmarshal([]byte(f), &postFound); err != nil {
				fmt.Println("Error", err)
			}
			posts = append(posts, postFound)
		}
		tmpl, _ := template.New("").ParseFiles("./tpl/admin/ann.gohtml", "./tpl/admin/base.gohtml", "./tpl/admin/nav.gohtml")
...
func AdminAnnouncementPageHandler(w http.ResponseWriter, r *http.Request) {
	// vars := mux.Vars(r)
	// lang := vars["lang"]
	// postID := vars["post"]
	userName := GetUserName(r)
